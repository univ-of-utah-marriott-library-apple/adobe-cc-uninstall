...
Adobe Creative Cloud uninstall workflow overview
...
History of Adobe uninstall on macOS
...
Adobe uninstall tool selection flow
...
Uninstall the app with AdobeUninstaller, a retained uninstall package, or a version-aware Setup command.
Use the Cleaner Tool only as a follow-up for broken installs, failed reinstalls, bad cleanup state, or difficult legacy cases.
Verify logs and filesystem remnants before reinstalling or declaring the Mac clean.
...
Adobe base-version evidence workflow
...
Expand or inspect the installer package first so you are searching the package evidence, especially optionXML.xml.
Search for the app's SAP code first, not baseVersion. For Bridge, search for KBRG; for Photoshop, search for PHSP; for Illustrator, search for ILST.
Confirm you are inside an <HDMedia> block for the target app.
Read the nearby fields together: SAPCode, prodVersion, mediaLEID, baseVersion, and productVersion.
Use the baseVersion from that same block in the uninstall command.
Do not use a baseVersion from a neighboring helper component, language pack, update component, or a different architecture block.
...
Suspicious Package search for KBRG showing the matching Bridge baseVersion in optionXML.xml
...
Sign in to GitHub with an account that has access to the repository.
...
Use Raw to view plain text, or Download to save a copy.
Use History to review line-by-line changes over time.
Access with GitHub Desktop
Open GitHub Desktop and sign in.
Select File > Clone repository.
Use the URL tab and enter:
https://github.com/univ-of-utah-marriott-library-apple/adobe-cc-uninstall.git

Choose a local destination and click Clone.
Click Show in Finder or Open in Visual Studio Code.
Open the script at repo root (adobe_cc_app_uninstaller.sh) or open the direct script page:
https://github.com/univ-of-utah-marriott-library-apple/adobe-cc-uninstall/blob/main/adobe_cc_app_uninstaller.sh
...
Inventory installed Adobe apps from both expected and custom paths.
Decide whether the goal is app removal, old-version cleanup, license cleanup, or repair.
Prefer Adobe's uninstall tool (AdobeUninstaller) for current supported Creative Cloud uninstall workflows.
Use retained uninstall packages when they are known to match the installed package generation.
Use version-aware Setup calls when you need per-title control or retained uninstallers are unavailable.
Verify SAP codes and base versions from optionXML.xml when documentation or behavior is questionable.
Treat the Cleaner Tool as repair/follow-up cleanup, not the main uninstaller.
Handle Adobe Genuine Service (AGS) and plug-ins separately when they are in scope.
Test on Intel and Apple Silicon Macs.
Log commands, exit codes, Adobe logs, app paths, and remaining filesystem evidence.
